...
public class Caja {
	...
	public void abrir() throws CajaException {
		if (abierta) {
			throw new CajaException("La caja " + numero + " ya est� abierta");
		}
		abierta = true;
	...
	public void cerrar() throws CajaException {
		if (!abierta) {
			throw new CajaException("La caja " + numero + " ya est� cerrada");
		}
		if (!clientes.isEmpty()) {
	...
	public int atenderCliente() throws CajaException {
		if (!abierta) {
			throw new CajaException("La caja " + numero + " est� cerrada");
		}
		if (clientes.isEmpty()) {
			throw new CajaException("No hay clientes en la caja " + numero);
		}
		int clienteAtendido = clientes.poll();
		System.out.println("Se ha atendido al cliente con n�mero " + clienteAtendido + " en la caja " + numero);
